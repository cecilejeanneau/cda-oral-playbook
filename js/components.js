...
async function initializeContent(techno){const contentContainer=document.createElement("div");contentContainer.className="container my-5";const homeButton=createHomeButton();const colorBtn=createColorButton();contentContainer.appendChild(homeButton);contentContainer.appendChild(colorBtn);await fetch("../json/data.json").then((response)=>response.json()).then((data)=>{const selectedData=data[techno];console.log(selectedData)
