# CDA Oral Playbook

Ce repository est un **fork** du projet original suivant :  
https://github.com/pharallaxe/doc_public

Ce fork a pour objectif personnel d’ajouter des précisions, compléments, reformulations, notes et enrichissements sur les contenus déjà présents, afin d’en faciliter l’étude, la mémorisation et l’appropriation dans le cadre de révisions personnelles.

Aucun changement structurel majeur n’est prévu à ce stade.  
L’idée est simplement de continuer / préciser / étoffer ce qui existe déjà.

---

## Licence

Ce projet reste sous licence **Apache License 2.0**.  
Le fichier `LICENSE` d’origine est conservé, et l’attribution initiale est maintenue conformément aux termes de la licence.

Les modifications éventuelles apportées seront identifiables dans l’historique Git.

---

## Auteur du fork

Fork maintenu par : https://github.com/cecilejeanneau
